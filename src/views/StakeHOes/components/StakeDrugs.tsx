...
const StakeDrugs: React.FC<StakeProps> = ({}) => {
  const tokenName = "DRUGS"
  const [requestedApproval, setRequestedApproval] = useState(false)

  const allowance = useAllowanceStaking()
  const {onApprove} = useApproveStaking()

  const tokenBalance = useTokenBalance(contractAddresses.drugs[97])
      ...
      <CardContent>
        <StyledCardContentInner>
          <StyledCardHeader>
            ...
            <Label text={`DRUGS Tokens Available`}/>
          </StyledCardHeader>
          <StyledCardActions>
            {!allowance.toNumber() ? (
              <Button
